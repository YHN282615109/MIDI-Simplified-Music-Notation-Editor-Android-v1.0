...
// 在player.h中添加声明
    void setMidiInstrument(int instrument);

// 在player.cpp中实现
    void setMidiInstrument(int instrument) {
        // 发送MIDI程序改变消息
        // 0xC0 = 程序改变消息 (通道0)
        // instrument = 乐器号 (0-127)
        unsigned char data[2];
        data[0] = 0xC0; // 程序改变，通道0
        data[1] = instrument & 0x7F; // 乐器号

        // 这里需要根据您的播放器实现来发送MIDI消息
        // 例如：synth->processMidiMessage(data, 2);
    }
...
void player::playStep(){
    if(playingStatus){
        ...
        if(round(tp-tempo)!=0){
            tempo=tp;
            ::__android_log_print(ANDROID_LOG_INFO, "midi","set tempo:%f\n",tempo);
        }
        ...
        for(auto it:toPlay){
            ...
            if(ins>=0){
                ...
                ::__android_log_print(ANDROID_LOG_INFO, "midi","fail to play note:%s(%d)\n",it->info.c_str(),(int)it->tone);
            }
        }
        toPlay.clear();
...
void player::noteOff(note * n){
    if(n){
        ...
        if(ins>=0){
            ...
            ::__android_log_print(ANDROID_LOG_INFO, "midi","fail to release note:%s(%d)\n",n->info.c_str(),(int)n->tone);
        }
    }
}
